...
def mainloop(myscr,cfg):
    ...
    while True:
        ...
        if c in ('CameraPrev', ord(',')):
            camera -= 1
            camera = camera % 8
            cameratext = 'Selecting camera view ' + str(camera + 1) + '...'
            statuswin.clear()
            statuswin.addstr(0,0,cameratext,curses.A_BOLD)
            statuswin.refresh()
            time.sleep(1)
            
        if c in ('CameraNext', ord('.')):
            camera += 1
            camera = camera % 8
            cameratext = 'Selecting camera view ' + str(camera + 1) + '...'
            statuswin.clear()
            ...
            try:
                ...
                if audio:
                    ...
                    if use_xml:
                        ...
                        if str(available[current_cursor][4]) in ('I', 'D', 'NB'):
